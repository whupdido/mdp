...
class PygameRenderer:
    ...
    def _viewport_for_size(self, width_px: int, height_px: int) -> WorldViewport:
        ...
        arena_width = (
            self.arena_width_cm
            if self.fastest_car_mode and self.arena_width_cm is not None
            else self.config.arena_size_cm
        )

        arena_height = (
            self.arena_height_cm
            if self.fastest_car_mode and self.arena_height_cm is not None
            else self.config.arena_height_cm or self.config.arena_size_cm
        )
    ...
    def _draw_header(self, section: PanelSection, editor: bool) -> None:
        ...
        if self.fastest_car_mode:
            title = "TASK 2 FASTEST CAR EDITOR" if editor else "TASK 2 FASTEST CAR SIMULATOR"
            subtitle = "Edit, plan, and play the Fastest Car route" if editor else "Deterministic Fastest Car playback"
        else:
            title = "TASK 1 EDITOR" if editor else "TASK 1 SIMULATOR"
            subtitle = "Edit, plan, and play a five-target route" if editor else "Deterministic command-aligned playback"
        self._blit_text(title, (x, y), self.TEXT, title=True)
